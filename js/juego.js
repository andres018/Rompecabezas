...
var instrucciones = [];
instrucciones.push("Utiliza las flechas del teclado para mover la pieza roja");
instrucciones.push("Debes ordenar las piezas hasta completar el objetivo");
...
function chequearSiGano() {
  ...
  let grillaActual = grilla; 
  
  for (let i = 0; i < grillaActual.length; i++) {
    for (let j = 0; j < grillaActual.length; j++) {
      if (grillaGanadora[i][j] !== grillaActual[i][j]){
        return false
      }
    }
  }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  let intercambiando = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1]=grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2]= intercambiando;
}
